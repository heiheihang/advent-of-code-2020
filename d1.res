...
let rec func1 = (~l as l, ~data as data) => {
    switch l {
        |list{} => {Js.log("sth")}
        |list{a, ...rest} => {
            let dict = Belt.Map.Int.fromArray([])
            let check = func2(~l = data, ~dict = dict, ~num = a)
            switch check {
                |0 => func1(~l = rest, ~data = data)
                |_ => Js.log(check)
            }
